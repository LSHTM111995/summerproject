...
mass=function(state,world,stock){
  effort=min(0.013*length(state[,1]),stock) #average number of vaccines attempted to be administered per day
...
update.behaviour=function(world,state){
  ...
  kappa=18 #sensitivity; larger values -> more sensitive
  alpha=0.05 #linear shift between 0 and 1; closer to 1 -> slower reaction time I think that 0.1 is most reasonable
...
simulate=function(size,reps=2,strat="none",game=FALSE){
  ...
  return(list(results,inst.trust,vac.trust,beh.trust,epi.curve.history,repro.history.complete))
}
