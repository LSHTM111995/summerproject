...
create.state=function(world,households){
  ...
  colnames(state)=c("Inst", "Inst", "Vac", "Beh", "S", "E", "I", "Rd", "Rr", "Rv", "Dur")
  ...
  for(i in 1:length(households)){
    value=sample(c(0.25,0.5,0.75,0.9),1,replace=TRUE, prob=c(0.1,0.2,0.3,0.4)) #trust 0.1,0.2,0.3,0.4 seems decent
    state[start:households[i],2]=value
    start=households[i]+1
  }
  #assign all other states around households
  state[,2]=state[,2]+runif(pop.size,-0.01,0.01) 
  state[,3]=state[,2]+runif(pop.size,-0.09,0.09)
  state[,4]=state[,2]+runif(pop.size,-0.09,0.09) 
...
mass=function(state,world,stock){
  effort=min(25,stock) #average number of vaccines attempted to be administered per day
...
expose=function(world, state, ser, trans){
  ...
  inf.prob=0.8 #probability of infection transmission given behaviour of node, beh < inf.prob -> infection
  ...
  number.dead.nei=colSums((state[,8]*state[,11])*adj)
...
update.behaviour=function(world,state){
  ...
  kappa=10 #sensitivity; larger values -> more sensitive
  alpha=0.1 #linear shift between 0 and 1; closer to 1 -> slower reaction time I think that 0.1 is most reasonable
  ...
  output[[1]]=inst.input
  output[[1]][updates]=inst.input[updates]-del*inst.del
  
  ### vaccine setion
  vacc.nei=which(colSums(state[,10]*adj)!=0)
  ...
  output[[2]]=vac.input
  output[[2]][vacc.nei]=output[[2]][vacc.nei]+0.002 #vaccination bias
  output[[2]][updates]=vac.input[updates]-del*vac.del
  
  ### behaviour section
  dead.nei=which(colSums(state[,8]*adj)!=0)
  ...
  output[[3]]=beh.input
  output[[3]][dead.nei]=output[[3]][dead.nei]+0.01 #funeral bias
  output[[3]][updates]=beh.input[updates]-del*beh.del
...
display=function(results,inst.trust,vac.trust,beh.trust,epi.curve.history,repro.history.complete){
  ...
  cc=round(mean(d[,1]),3)
  cc.lower=round(mean(d[,1])-sqrt(mean(d[,1])*(1-mean(d[,1]))/length(d[,1]))*1.96, 3)
  cc.upper=round(mean(d[,1])+sqrt(mean(d[,1])*(1-mean(d[,1]))/length(d[,1]))*1.96,3)
  ...
  #r=round(rowMeans(repro.history.complete[1,-1],na.rm=TRUE),3)
  r.list=unname(unlist(repro.history.complete[1:2,-1]))
  r.list=r.list[which(r.list!=0)]
  r=round(mean(r.list),3)
  # r.lower=round(r-1.96*sd(repro.history.complete[1,-1])/length(repro.history.complete[1,-1]),3)
  # r.upper=round(r+1.96*sd(repro.history.complete[1,-1])/length(repro.history.complete[1,-1]),3)
...
plot.epi.curve=function(history,control.start,stockout.time,restock.time){
  ...
  plot(smooth.average, type = "l", ylim = c(0,max(smooth.upper)+1),
       main=paste("Average Epidemic Curve of",num.runs,"Outbreaks"),ylab="Prevalent Cases",xlab="Day of Outbreak")
  polygon(c(index,rev(index)),
          c(smooth.lower,rev(smooth.upper)),col = "grey75", border = FALSE)
  lines(smooth.average,col='red',lwd=2)
...
plot.repro=function(repro.history.complete){
  ...
  plot(averages,type='l',col='grey',ylab="Reproduction Number", xlab="Time", main=paste("Average Reproduction Number of",num.runs,"Outbreaks"))
  lines(smooth.repro,type='l',col='red',lwd=2)
  abline(1,0,lty=3)
  abline(mean(averages),0,col='blue')
}
...
outbreak=function(world, state, strat, game){
  ...
  mort=mortality.rate()
  ser=15 #avergae serial interval in days
  infdur=12 #average infectious duration in days 
  ...
  threshold=15 #number of cases required before vaccination begins
  primaries=3 #number of primary index cases to seed outbreak SHOULD BE LOWER THAN THRESHOLD
  control.limit=1 #number of cases allowed remaining for outbreak to "end" MUST BE LOWER THAN PRIMARIES
  init.stock=100 #quantity of vaccines available until 30 days pass, lag DEFINED ABOVE inside stock function
  delay=30
  ...
  control.duration=0
  duration=0
  infected.nodes=primaries#append(infected.nodes,index.cases)
  epi.curve=rbind(epi.curve,c(duration,infected.nodes))
  repro.history=c()#matrix(0,1,2)
  ...
  while(infected.nodes<threshold & sum(state[,6],state[,7])>control.limit){
    ...
    if(moving.to.removed>0){
      state[(state[,7]==1 & state[,11]==0),c(7,8,9,11)]= matrix(replicate(moving.to.removed,c(0,sample(0:1,2,prob=(c((1-mort),mort))),2)),
                                                                moving.to.removed,4,byrow=TRUE)
    }
    recovered.nodes=sum(state[,c(8,9)])
    #decrease waiting time and increase clock time
    state[,11]=state[,11]-1 #duration will then act as negative clock for those removed (may be interesting????)
    repro.history=rbind(repro.history,exposures[[2]])#c(duration,exposures[[2]]))
    duration=duration+1
  ...
  while(sum(state[,6],state[,7])>control.limit){
    ...
    if(moving.to.infected.short>0){
      state[(state[,6]==1 & state[,11]==0 & state[,2]>0.3),c(6,7,11)]=matrix(replicate(moving.to.infected.short,c(0,1,short.infection.duration(infdur))),
  ...
  output=list(state,epi.curve,repro.history,stock.status)
  return(output)
}
...
simulate=function(size,reps=2,strat="none",game=FALSE){
  ...
  for(k in 1:reps){
    ...
    repro.history.complete=cbind.fill(repro.history.complete,output[[3]])
  ...
  return(list(results,epi.curve.history,repro.history.complete))
}