...
expose=function(world, state, ser, trans){
  r=0.8 #small placeholder to further modify transmission dynamics
  ...
  funeral.exposures=funeral.exposures[which(state[funeral.exposures,4]<0.0)]
...
plot.repro=function(repro.history.complete){
  ...
  l=loess.sd(averages ~ index, span=0.17, nsigma = 1.96)
  plot(l$x, l$y,type='l', ylim = c(0,max(l$upper)),
    main=paste("Average Reproduction Number of",num.runs,"Outbreaks"),ylab="Reproduction Number",xlab="Day of Outbreak")
  polygon(c(index,rev(index)),
    c(l$lower,rev(l$upper)),col = "grey75", border = FALSE)
  lines(l$y,col='red',lwd=2)
  abline(v=min(which(l$y<=1)),lty=3)
  abline(1,0,lty=3)
...
outbreak=function(world, state, strat, game){
  ...
  mort=mortality.rate()
  ser=12 #avergae serial interval in days
